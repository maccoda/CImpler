...
/// Checks if a file exists. Will return false if it exists or it exists but is
/// a directory.
pub fn check_file_exists<P: AsRef<Path>>(path: P) -> bool {
    fs::metadata(path).map(|x| !x.is_dir()).unwrap_or(false)
}

/// Checks if a directory exists. Will return false if it exists or it exists
/// but is a file.
pub fn check_dir_exists<P: AsRef<Path>>(path: P) -> bool {
    fs::metadata(path).map(|x| x.is_dir()).unwrap_or(false)
}

/// Copies `file_name` located in `source_dir` across to `dest_dir` under the
/// same name.
